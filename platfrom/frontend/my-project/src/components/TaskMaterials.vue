    ...
    <div class="container">
      ...
      <main class="main-content">
        <div v-if="task">
          ...
          <p>{{ task.text }}</p> <!-- Здесь отображается текст занятия -->

          <!-- Отображение изображений -->
          <div v-if="images.length" class="images-container">
            <h3>Изображения:</h3>
            <div class="images">
              <img v-for="image in images" :src="image" :alt="task.name" :key="image" />
            </div>
          </div>
...
export default {
  ...
  if (this.task.files) {
    ...
    const allFiles = Array.isArray(this.task.files)
      ? this.task.files
      : this.task.files.split(",");
